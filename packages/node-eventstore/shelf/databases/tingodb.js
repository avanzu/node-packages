...
_.extend(Tingo.prototype, {
    connect: function (callback) {
        ...
        this.events.ensureIndex({ aggregateId: 1, streamRevision: 1 }, function (err) {
            if (err) {
                debug(err)
            }
        })
        ...
        this.events.ensureIndex({ dispatched: 1 }, { sparse: true }, function (err) {
            if (err) {
                debug(err)
            }
        })
        ...
        this.snapshots.ensureIndex({ aggregateId: 1, revision: -1 }, function (err) {
            if (err) {
                debug(err)
            }
        })
