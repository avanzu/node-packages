...
_.extend(Redis.prototype, {
        ...
        this.heartbeatInterval = setInterval(function () {
            var graceTimer = setTimeout(function () {
                if (self.heartbeatInterval) {
                    console.error(
                        new Error('Heartbeat timeouted after ' + gracePeriod + 'ms (redis)').stack
                    )
                    self.disconnect()
                }
            }, gracePeriod)
    ...
    addEvents: function (events, callback) {
        ...
        multi.exec(function (error, revisions) {
            ...
            if (errors.length) {
                var message =
                    'error while adding events for aggregate ' + aggregate + ' ' + aggregateId
    ...
    getEventsSince: function (date, skip, limit, callback) {
        ...
        this.scan(
            this.options.prefix + ':' + this.options.eventsCollectionName + ':*:*:*:*:*:*',
            function (keys, fn) {
    ...
    cleanSnapshots: function (query, callback) {
        ...
        this.scanSnapshots(query, function (error, keys) {
            ...
            var keysToDelete = keys.sort().slice(0, -1 * self.options.maxSnapshotsCount)
