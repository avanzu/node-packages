...
_.extend(DynamoDB.prototype, {
    connect: function (callback) {
        ...
        async.parallel(
            [createEventsTable, createSnapshotTable, createUndispatchedEventsTable],
            function (err) {
                if (err) {
            ...
            function (event, callback) {
                var results = [
                    function (callback) {
                        ...
                        debug(
                            'Saving event to events table: ' + JSON.stringify(storedEvent, null, 2)
                        )
                ...
                if (self.options.useUndispatchedEventsTable) {
                    ...
                    results.push(function (callback) {
                        ...
                        self.documentClient.put(storedEvent, function (err, data) {
                            if (err) {
                                error(
                                    'dynamodb.addUndispatchedEvents error: ' + JSON.stringify(err)
                                )
                                return callback(err)
                            } else {
    ...
    getLastEvent: function (query, callback) {
        ...
        async.doWhilst(
            ...
            function (err) {
                ...
                entities = _.sortBy(entities, function (e) {
                    return [new Date(e.commitStamp).getTime(), e.streamRevision, e.commitSequence]
                }).reverse()
        ...
        client.describeTable({ TableName: p.TableName }, function (err, data) {
            if (err) {
                if (err.code === 'ResourceNotFoundException') {
                    debug(
                        'Table ' + p.TableName + " doesn't exist yet: " + JSON.stringify(p, null, 2)
                    )
                ...
                if (err && !isTableAlreadyExistsError(err)) {
                    ...
                    cbCreate(null, {
                        Table: {
                            ...
                            TableStatus: data ? data.TableDescription.TableStatus : 'UNKNOWN',
                        },
                    })
                }
            })
        ...
        client.describeTable({ TableName: name }, function (err, data) {
            if (err) {
                if (err.code === 'ResourceNotFoundException') {
                    ...
                    error(
                        'deleteTableIfExists - describeTable error: ' + JSON.stringify(err, null, 2)
                    )
                    cbExists(err)
                }
            } else {
...
var clearEventTables = function (opts, documentClient, cleared) {
    ...
    async.eachSeries(
        ...
        function (t, afterEach) {
            async.doWhilst(
                function (next) {
                    if (exclusiveStartKey) t.params.ExclusiveStartKey = exclusiveStartKey
...
var deleteAllTempTables = function (client, opts, done) {
    ...
    var read = function (query, callback) {
        if (exclusiveStartTableName) query.ExclusiveStartTableName = exclusiveStartTableName
