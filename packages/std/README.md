...
```js
Result.Ok('foo').map(s => s.toUpperCase()).unwrapOr('') 
// -> 'FOO' 
Result.Err().map(s => s.toUpperCase()).unwrapOr('') 
// -> ''
```
### Monad 
 > `T.chain(a -> T(b)) -> T(b)`
...
```js
Result.Ok('foo').chain(s => Result.Ok(`${s}bar`)).unwrapOr('')
// -> foobar

Result.Ok('foo').chain(s => Result.Err('Does not compute.')).unwrapOr('')
// -> ''
```
